...
```js
/**
 * @typedef {string} OrderState - 订单状态，有 7 种定义
 * "PAYING"  - 【待支付】未支付，正在等待支付
 * "PAID"    - 【已支付】已完成支付，但未打印，正在等待打印
 * "PRINTED" - 【已打印】已完成打印，但未取件，等待配送或取件
 * "FINISH"  - 【已取件】完成取件
 * "CANCEL"  - 【已取消】取消订单（已支付的订单不可取消）
 * "REFUNDING" - 【待退款】正在退款（已打印的订单不可退款）
 * "REFUNDED"  - 【已退款】
 */
```
